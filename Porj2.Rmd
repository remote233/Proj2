...
train_cv = rbind(train_method2,validation_method2,test_method2)
train_cv$new_blocks <- match(train_cv$blocks, unique(train_cv$blocks))
# number of folds
K=10
train_cv = train_cv %>% mutate(folds = new_blocks%%K+1, expert_label = ifelse(expert_label==-1,0,1))
train_feature = train_cv %>% dplyr::select(NDAI,SD,CORR)
train_label = train_cv %>% dplyr::select(expert_label)
train_folds = train_cv %>% dplyr::select(folds)
#try different models here
CVmaster(lr_model, train_feature, train_label, train_folds)
```
