...
ggplot(image3_filtered,aes(x,y,color=expert_label))+
  ...
  labs(colour="expert_label")+
  geom_hline(yintercept=c(lim1,lim2),color="red")
```
...
### Second Method
In the second method, we can use blocks to split the all the images and randomly select 70% blocks as our training set, 20% as validation set, and the remaining 10% as test set. We plan to divide each image into 100 blocks and then we will have totally 300 blocks. Then, we can divide those 300 blocks into train, validation, and test sets according to the ratio we set. For both methods, we will delete unlabeled pixels after splitting the data because they are not useful for our classification.
```{r}
set.seed(111)
...
pred_prob = predict(train_xgb1, xgb_test)
pred = rep(0,length(pred_prob))
pred[pred_prob>0.5] = 1
test_accuracy[6,1] <- round(mean(pred==test1$expert_label),3)

xgb_train = xgb.DMatrix(data = data.matrix(train2[,c("NDAI","SD","CORR")]), label = train2$expert_label)
xgb_test = xgb.DMatrix(data = data.matrix(test2[,c("NDAI","SD","CORR")]), label = test2$expert_label)
watchlist = list(train=xgb_train, test=xgb_test)
model = xgb.train(data = xgb_train, max.depth = 3, watchlist=watchlist, nrounds = 1000, verbose = 0)
final_rounds = as.integer(model$evaluation_log[which.min(test_rmse),1])
train_xgb2 = xgboost(data = xgb_train, max.depth = 3, nrounds = final_rounds, verbose = 0)
pred_prob = predict(train_xgb2, xgb_test)
pred = rep(0,length(pred_prob))
pred[pred_prob>0.5] = 1
...
```
## 3(b)
The cut-off value is the point on the curve that is cloest to the upper-left corner point, (0,1) on the graph.
```{r}
## All roc curve are based on split method2
alpha=seq(0.01,0.99,0.01)
# Logistic_regression
FPR1=c()
TPR1=c()
for(i in 1:99){
  pred=rep(0,nrow(test2))
  pred[lr_pred2>alpha[i]]=1
  
  obs=test2$expert_label
  table=table(obs,factor(pred,level=0:1))
  
  ## sum of 1st column is pred's label=0, sum of 2nd column is pred's label=1, 
  FPR1[i]=table[1,2]/(table[1,1]+table[1,2])
  TPR1[i]=table[2,2]/(table[2,1]+table[2,2])
}
```
...
TPR2=c()
for(i in 1:99){
  nb_pred2 = predict(nb_mod2, test2, type="raw")
  pred=rep(0,nrow(test2))
  pred[nb_pred2[,2]>alpha[i]]=1
  
  obs=test2$expert_label
  table=table(obs,factor(pred,level=0:1))
  
  ## sum of 1st column is pred's label=0, sum of 2nd column is pred's label=1, 
  FPR2[i]=table[1,2]/(table[1,1]+table[1,2])
  TPR2[i]=table[2,2]/(table[2,1]+table[2,2])
}
```
...
for(i in 1:99){
  ...
  pred=rep(0,nrow(test2))
  pred[qda_pred2$posterior[,2]>alpha[i]]=1
  
  obs=test2$expert_label
  table=table(obs,factor(pred,level=0:1))
  
  ## sum of 1st column is pred's label=0, sum of 2nd column is pred's label=1, 
  FPR3[i]=table[1,2]/(table[1,1]+table[1,2])
  TPR3[i]=table[2,2]/(table[2,1]+table[2,2])
}
```
...
for(i in 1:99){
  ...
  pred=rep(0,nrow(test2))
  pred[lda_pred2$posterior[,2]>alpha[i]]=1
  
  obs=test2$expert_label
  table=table(obs,factor(pred,level=0:1))
  
  ## sum of 1st column is pred's label=0, sum of 2nd column is pred's label=1, 
  FPR4[i]=table[1,2]/(table[1,1]+table[1,2])
  TPR4[i]=table[2,2]/(table[2,1]+table[2,2])
}
```
...
set.seed(12345)
#knn_mod2 <- knn(train2[,c("NDAI","SD","CORR")], test2[,c("NDAI","SD","CORR")], train2$expert_label, k=20)
train_knn_roc = roc(factor(test2$expert_label,ordered = TRUE),as.numeric(knn_mod2),plot = TRUE,print.auc = TRUE,print.thres = 0.5,main = "ROC for KNN")
```
...
TPR6=c()
for(i in 1:99){
  pred_prob = predict(train_xgb2, xgb_test,outputmargin=FALSE)
  pred=rep(0,nrow(test2))
  pred[pred_prob>alpha[i]]=1
  
  obs=test2$expert_label
  table=table(obs,factor(pred,level=0:1))
  
  ## sum of 1st column is pred's label=0, sum of 2nd column is pred's label=1, 
  FPR6[i]=table[1,2]/(table[1,1]+table[1,2])
  TPR6[i]=table[2,2]/(table[2,1]+table[2,2])
}
```


```{r}
df=data.frame(FPR=c(FPR1,FPR2,FPR3,FPR4,FPR6),TPR=c(TPR1,TPR2,TPR3,TPR4,TPR6),method=rep(c("logistic_reg","naive_bayes","QDA","LDA","XGBoost"),each=99))
ggplot(data = df, aes(x=FPR, y=TPR)) + geom_line(aes(colour=method))
```
```{r}
library(e1071)
library(ROCR)
probs <- predict(nb_mod2, test2, type="raw")
pred <- prediction(probs[, 2], test2$expert_label)
perf_nb <- performance(pred, measure='tpr', x.measure='fpr')
roc_nb <- data.frame(fpr=unlist(perf_nb@x.values), tpr=unlist(perf_nb@y.values))
roc_nb$method <- "naive bayes"
ggplot(data=roc_nb,aes(x=fpr, y=tpr, linetype=method, color=method)) + 
  geom_line() +
