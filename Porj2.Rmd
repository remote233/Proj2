...
pred = predict(nb_mod1, test1)
test_accuracy[2,1] = round(mean(pred==test1$expert_label),3)
nb_mod2 <- naiveBayes(form, data=train2)
pred = predict(nb_mod2, test2)
test_accuracy[2,2] = round(mean(pred==test2$expert_label),3)

# QDA
qda_mod1 <- qda(form, data=train1)
pred = predict(qda_mod1, test1)
test_accuracy[3,1] = round(mean(pred$class==test1$expert_label),3)
qda_mod2 <- qda(form, data=train2)
pred = predict(qda_mod2, test2)
test_accuracy[3,2] = round(mean(pred$class==test2$expert_label),3)

# LDA
lda_mod1 <- lda(form, data=train1)
pred = predict(lda_mod1, test1)
test_accuracy[4,1] = round(mean(pred$class==test1$expert_label),3)
lda_mod2 <- lda(form, data=train2)
pred = predict(lda_mod2, test2)
test_accuracy[4,2] = round(mean(pred$class==test2$expert_label),3)
