...
## (c)
In order to determine 3 best features out of 8 features, we decided to use each feature to build a single-predictor logistics regression model based on training data set from both split method1 and split method2, and use the validation data set from from both split method1 and split method2 to draw the ROC curve.
We will get two area under the ROC curve (AUC) values, and make them equal weighted to get the average AUC value to evaluate the overall performance of each model. 
An ideal ROC curve will huge the top left corner, so the larger the average AUC value the best the classifier, we will choose three features with the first three largest average AUC value as the best three features in this problem.
```{r}
...
library(gridExtra)
par(mfrow=c(2,4))
p1=ggplot(train_method1,aes(x=NDAI,fill=expert_label))+
  geom_density(alpha=0.5)
p2=ggplot(train_method1,aes(x=SD,fill=expert_label))+
  geom_density(alpha=0.5)
p3=ggplot(train_method1,aes(x=CORR,fill=expert_label))+
  geom_density(alpha=0.5)
p4=ggplot(train_method1,aes(x=DF,fill=expert_label))+
  geom_density(alpha=0.5)
p5=ggplot(train_method1,aes(x=CF,fill=expert_label))+
  geom_density(alpha=0.5)
p6=ggplot(train_method1,aes(x=BF,fill=expert_label))+
  geom_density(alpha=0.5)
p7=ggplot(train_method1,aes(x=AF,fill=expert_label))+
  geom_density(alpha=0.5)
p8=ggplot(train_method1,aes(x=AN,fill=expert_label))+
  geom_density(alpha=0.5)
...
library(gridExtra)
par(mfrow=c(2,4))
p1=ggplot(train_method2,aes(x=NDAI,fill=expert_label))+
  geom_density(alpha=0.5)
p2=ggplot(train_method2,aes(x=SD,fill=expert_label))+
  geom_density(alpha=0.5)
p3=ggplot(train_method2,aes(x=CORR,fill=expert_label))+
  geom_density(alpha=0.5)
p4=ggplot(train_method2,aes(x=DF,fill=expert_label))+
  geom_density(alpha=0.5)
p5=ggplot(train_method2,aes(x=CF,fill=expert_label))+
  geom_density(alpha=0.5)
p6=ggplot(train_method2,aes(x=BF,fill=expert_label))+
  geom_density(alpha=0.5)
p7=ggplot(train_method2,aes(x=AF,fill=expert_label))+
  geom_density(alpha=0.5)
p8=ggplot(train_method2,aes(x=AN,fill=expert_label))+
  geom_density(alpha=0.5)
grid.arrange(p1,p2,p3,p4,p5,p6,p7,p8,ncol=2)
```
...
```{r}
tab <- matrix(c(m1_auc,m2_auc,avg_auc), ncol=3)
