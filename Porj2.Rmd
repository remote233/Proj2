...
# 3 Modeling
## (a)
We decide to try classification methods including logistic regression, SVM, naive Bayes, LDA and QDA.
```{r}
loss_function <- function(pred_result){
  pred_result %>%
    mutate(accuracy = ifelse(pred==obs,1,0))%>%
    group_by(Resample) %>%
    summarise(Accuracy = round(mean(accuracy),3))
}
train_CV_1 = rbind(train_method1, validation_method1) %>%
  mutate(expert_label = ifelse(expert_label==-1,0,1))
train_feature1 = train_CV_1 %>% select(NDAI,SD,CORR)
train_label1 = train_CV_1 %>% select(expert_label)
set.seed(12345)
CVmaster("lda",train_feature1,train_label1,K=10,loss_function)
```
